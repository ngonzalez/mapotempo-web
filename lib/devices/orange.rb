    ...
    xml.tag! :ROOT do
      ...
      xml.tag! :zone, nil, type: 'operation' do
        route.stops.select(&:active?).select(&:position?).select(&:time?).sort_by(&:index).each do |stop|
          start_time = stop.time - 2.hours
          xml.tag! :operation, nil, options.merge(seq: stop.index, ad1: stop.street, ad2: nil, ad3: nil, ad_zip: stop.postalcode,
            ad_city: stop.city, ad_cntry: ("FR" || stop.country), latitude: stop.lat, longitude: stop.lng, title: stop.name, txt: [stop.street, stop.postalcode, stop.city].join(", "),
            prevopedeb: p_time(route, start_time).strftime("%d/%m/%Y %H:%M"), prevopefin: p_time(route, start_time).strftime("%d/%m/%Y %H:%M"))
        end
      end
    end
  end
